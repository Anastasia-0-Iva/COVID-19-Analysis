...
path = r'C:\Users\Анастасия\OneDrive\Desktop\owid-covid-data.csv'
df = pd.read_csv(path, delimiter=',', encoding='latin-1')
df['year'] = pd.to_datetime(df['date'], errors='coerce').dt.year

# Функции для обработки данных
def divide_the_year(df):
    return df.groupby('year')

def divide_the_countries(df):
    return df['location'].unique()


def number_of_cases(dataframes, countries):
    results = []
...
def calculate_stats(df, year, country):
